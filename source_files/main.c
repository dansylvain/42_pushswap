...
/*   Updated: 2023/12/03 16:42:38 by dan              ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	main(int argc, char **argv)
{
	...
	if (pile_a_is_reversed(data->pile_a) && !pile_a_is_ordered(data->pile_a))
		instruction_swap_single(data->pile_a, sa);
	if (!get_lis_tab(data->pile_a, &lis_tab, data))
		return (error_message(), free_all(data, input_tab, lis_tab, argv));
	calculate_pivots(data, input_tab);
	pre_sort_pile_a(data, lis_tab);
	sort_piles(data);
